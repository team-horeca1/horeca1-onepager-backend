...
// Cache for the logo buffer to avoid repeated disk reads (CPU optimization)
let logoCache = {
  path: null,
  buffer: null,
  lastChecked: 0
};
const LOGO_CACHE_TTL = 3600000; // 1 hour
...
// Get the logo path - handles both local and Vercel serverless environments
// Optimized with caching to avoid repeated sync fs checks (CPU optimization)
const getLogoPath = () => {
  const now = Date.now();
  if (logoCache.path && (now - logoCache.lastChecked < LOGO_CACHE_TTL)) {
  ...
  for (const logoPath of possiblePaths) {
    if (fs.existsSync(logoPath)) {
      logoCache.path = logoPath;
      logoCache.lastChecked = now;
      return logoPath;
    }
  }

  console.warn('[Email] Logo file not found in any expected location');
  return null;
};
...
const generateHeader = (doc, invoice) => {
  ...
  // Try to add logo with error handling for serverless environments
  try {
    ...
    console.warn('[Email] Could not add logo to PDF:', logoError.message);
    // Continue without logo - the PDF will still be generated
  }
...
function generateCustomerInformation(doc, invoice) {
  ...
  doc
    ...
    .text(`${invoice?.user_info?.city || ""}, ${invoice?.user_info?.country || ""} ${invoice?.user_info?.zipCode || ""}`, 200, 200, { align: "right" })
    .text(invoice?.user_info?.phone, 200, 215, { align: "right" });
